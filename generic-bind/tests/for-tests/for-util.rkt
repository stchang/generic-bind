...
(define-syntax-rule (test-values seqs e)
  (call-with-values e (λ args (for-each check-equal? args seqs))))
;    (check-equal? x (car seqs))
;    (check-equal? y (cadr seqs))))
    ...
    [(_ [seq] gen) ; we assume that seq has at least 2 elements, and all are unique
     (begin
       ...
       (test `seq 'gen (reverse (for/fold ([a null]) ([i gen]) 
                                  (cons i a))))
       (test `seq 'gen (let ([(~vs more? next) (sequence-generate gen)])
                         (let loop ()
                           (if (more?)
                               (cons (next) (loop))
                               null))))
       ...
       (let ([count 0])
         ...
         (let ([second (for/last ([(i pos) (in-parallel gen (in-naturals))] #:when (< pos 2))
           ...
           (check-equal? second (list-ref `seq 1))
           (test 2 values count)
