...
(struct A (x y))
(~define ($ (A x5 y5)) (A 101 202))
(check-equal? x5 101) (check-equal? y5 202)
(~define (⋈ v1 v2) (values 987 654))
...
(~define (g ($ (list (list a b) y ...))) (apply + a b y))
(check-equal? (g (list (list 101 202) 303)) 606)

; (~define (gv (:=v x y)) (+ x y))) ; expand fail because of invalid :=v bind
...
(~define (⋈ ($ (list xxxx yyyy)) zzzz) (values (list 11 22) 33))
(check-equal? xxxx 11)
(check-equal? yyyy 22)
(check-equal? zzzz 33)
...
(check-equal? (~let ([($ (list x y)) (list 10 20)]) (+ x y)) 30)
(check-equal? (~let ([($ (list x y)) (list 10 20)] [($ (cons a b)) (cons 40 50)]) (+ x y a b)) 120)
(check-equal? (~let ([(⋈ v1 v2) (values 10 20)]) (+ v1 v2)) 30)
(check-equal? (~let ([(⋈ ($ (list ab cd))) (values (list 99 88))]) (- cd ab)) -11)
(check-equal? 
 (~let ([(⋈ ($ (list ab cd)) ($ (cons ef fg)))
         (values (list 99 88) (cons 77 66))]) 
       (+ (- cd ab) (- fg ef)))
 -22)
(check-equal? (~let loop ([($ (list x y ...)) (list 1 2 3 4 5)]) (if (null? y) x (+ x (loop y)))) 15)